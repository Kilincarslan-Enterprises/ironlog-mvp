...
(Settings → Environment variables) — see [`docs/secrets.md`](docs/secrets.md)
for the full setup. D1 migrations are applied separately by the
`Apply D1 migrations (remote)` GitHub Actions workflow (two repo secrets).
